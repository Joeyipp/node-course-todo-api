...
// // Before each testing lifecycle, remove all entries in the db
// beforeEach((done) => {
//   Todo.remove({}).then(() => {
//     done();
//   });
...
describe('GET /todos/:id', () => {
  ...
  it('should not return todo doc created by other user', (done) => {
    request(app)
      .get(`/todos/${todos[1]._id.toHexString()}`) // toHexString converts ObjectID to string
      .set('x-auth', users[0].tokens[0].token)
      .expect(404)
      .end(done);
  });
...
describe('PATCH /todos/:id', () => {
  it('should update the todo', (done) => {
    ...
    // patch, send, update text, set completed = true
    // 200
    // res.body has text property = text, completed is true, completedAt is a number
    var hexId = todos[0]._id.toHexString();
...
describe('POST /users', () => {
    ...
    request(app)
      ...
      .end((err) => { // Custom end function
        ...
        User.findOne({email}).then((user) => {
          ...
          expect(user.password).not.toBe(password); // toNotBe()
          done();
        }).catch((e) => done(e));
      });
  });
  ...
  it('should return validation errors if request invalid', (done) => {
    request(app)
      ...
      .send({
        ...
        password: 'abc' // Password minlength is 6
      })
      .expect(400)
      .end(done);
  });
  ...
  it('should not create user if email in use', (done) => {
    request(app)
      .post('/users')
      .send({
        email: users[0].email, // Duplicate email (not unique)
        password: '123abc!'
      })
      .expect(400)
      .end(done);
